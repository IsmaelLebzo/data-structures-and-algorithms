...
const screenForNames = (arr) => {
  let regex = /^(mr. )|(mrs. )|(ms. )|(dr. )/gi;
  let noRegex = /^(mr.  )/gi;
  let newArr = arr.filter(value => regex.test(value) && !noRegex.test(value) ? true:false);
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  let newArr = [];
  arr.length === 0 ? [] : arr.forEach(value => value.mass > 77 ? newArr.push(value.name) : false);
  return newArr.join(' - ');
};
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  let testUrl = url.includes('https://') ? true : false;
  return testUrl;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6 

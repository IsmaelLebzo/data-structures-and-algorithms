...
const sortByChildren = (charArray) => {
  let newArr = charArray.sort((a,b) => a.children.length - b.children.length);
  return newArr;
};
...
const containsWorld = (input) => {//that was easy
...
const isCapitalized = (str) => {//that was a hard one!
  let check = /\b[A-Z]\w*/g;
  let str1;
  if (check.test(str)){
    str1 = str.match(check);
  }else{
    str1 = [];
  }
  return str1;
...
const citiesAtoJ = (arr) => {
  let check = /^[A-J]/;
  return arr.filter(value => check.test(value));
};

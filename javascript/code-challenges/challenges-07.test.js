...
const listFoods = (recipe) => {
  let result = [];
  recipe.ingredients.map(value => result.push(value.slice(value.indexOf(' ')+1,value.length)));
  let newArr =[];
  result.map(value => newArr.push(value.slice(value.indexOf(' ')+1,value.length)));
  console.log(result);
  console.log(newArr);
  return newArr;
};

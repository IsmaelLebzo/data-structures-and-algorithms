...
function transformToLis(obj){
  ...
  // console.log(newArr);
  let endArr = newArr.map(value =>{
    let obje = `<li>${value[0]}: ${value[1]}</li>`;
    return obje;
  });
  return endArr;
}
...
const divisibleByFiveTwoToThePower = (input) => {
  let newArr = input.map(value =>
    value.filter(value =>
      typeof value === 'number' && value % 5 === 0 ? true : false).map(num =>
      Math.pow(2, num)));
  return newArr;
};
...
let findMaleAndFemale = (data) => { // this one was long and kinda hard
  let result = data.reduce((acc, value) => {
    if (value.gender === 'female' || value.gender === 'male'){
      if(acc === ''){
        acc = value.name;
      }else{
        acc = acc + ' and ' + value.name;
      }
    }
    return acc;
  }, '');
...
let findShortest = (data) => {
  let test1 = data.reduce((acc , value) =>
    acc < value.height ? acc = value.height : acc , 0 );
  let test2 = data.reduce((acc, value) =>
    value.height === test1 ? acc = value.name : acc, '');
  return test2;
};

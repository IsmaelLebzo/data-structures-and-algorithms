...
const findHappiness = (arr) => {
  ...
  arr.forEach(value => {
    if (value.match(regex) !== null){
      newArr.push(value.match(regex)[0]);
    }
  });
  return newArr;
};
...
const onlyOddChars = (str) => {
  let newArr = [];
  str.split('').reduce((ele, value, acc) => acc%2 ===1? newArr.push(value): '',[]);
  return newArr.join('');
};
...
const allHappy = (arr) => {
  return arr.length === arr.filter (value => value.includes(':)')).length;
};
